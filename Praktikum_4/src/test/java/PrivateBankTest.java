...
class PrivateBankTest {
    ...
    @ParameterizedTest
    @ValueSource(doubles = {500.0, -200.0, 0.0, 1500.0, -1000.0})
    void testAccountBalanceParameterized(double amount) throws TransactionAlreadyExistException, AccountAlreadyExistsException, TransactionAttributeException, IOException {
        // Create an account and add a transaction with the given amount
        bank1.createAccount("testAccount", new ArrayList<>(List.of(new Payment("15.12.2024", amount, "Parameterized Test"))));

        // Calculate expected balance
        double expectedBalance = amount; // Only one transaction, so balance should equal the amount

        // Verify account balance
        assertEquals(expectedBalance, bank1.getAccountBalance("testAccount"));
    }
